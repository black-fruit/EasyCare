...
class Net {
    ...
    setupOnce(_addGlobalEventProcessor, getCurrentHub) {
        ...
        (0, utils_1.fill)(electron_1.net, 'request', createWrappedRequestFactory(this._options, clientOptions === null || clientOptions === void 0 ? void 0 : clientOptions.tracePropagationTargets));
    }
}
exports.Net = Net;
/** @inheritDoc */
...
function addHeadersToRequest(request, url, sentryTraceHeader, dynamicSamplingContext) {
    utils_1.logger.log(`[Tracing] Adding sentry-trace header ${sentryTraceHeader} to outgoing request to "${url}": `);
    request.setHeader('sentry-trace', sentryTraceHeader);
    const sentryBaggageHeader = (0, utils_1.dynamicSamplingContextToSentryBaggageHeader)(dynamicSamplingContext);
...
function createWrappedRequestFactory(options, tracePropagationTargets) {
    ...
    const createSpanUrlMap = new lru_map_1.LRUMap(100);
    const headersUrlMap = new lru_map_1.LRUMap(100);
    ...
    const shouldAttachTraceData = (method, url) => {
        ...
        if (tracePropagationTargets) {
            const decision = (0, utils_1.stringMatchesSomePattern)(url, tracePropagationTargets);
    ...
    return function wrappedRequestMethodFactory(originalRequestMethod) {
        return function requestMethod(reqOptions) {
            ...
            let span;
            const hub = (0, node_1.getCurrentHub)();
            ...
            if (scope && shouldCreateSpan(method, url)) {
                ...
                if (parentSpan) {
                    ...
                    if (shouldAttachTraceData(method, url)) {
                        ...
                        const sentryTraceHeader = (0, utils_1.generateSentryTraceHeader)(traceId, undefined, sampled);
                        const client = hub.getClient();
                        const dynamicSamplingContext = dsc || (client ? (0, core_1.getDynamicSamplingContextFromClient)(traceId, client, scope) : undefined);
                        addHeadersToRequest(request, url, sentryTraceHeader, dynamicSamplingContext);
                    }
                }
            }
...
function addRequestBreadcrumb(event, method, url, req, res) {
    (0, node_1.getCurrentHub)().addBreadcrumb({
        type: 'http',
